...
class CommentService:
    def __init__(
            ...
            blog_post_service: Annotated[BlogService, Depends()],
        ) -> None:
            self._sess = session
            self.auth_service = auth_service
            self.blog_service = blog_post_service
    ...
    async def create(self, body: create_schema, user_id: int):
        ...
        if not user:
             return 
        blog_post = await self.blog_service.get_one(body.blog_post_id)
        if not blog_post:
             return
        db_model.creator = user
        if not blog_post.comments:
             blog_post.comments=[]
        blog_post.comments.append(db_model)
    ...
    async def get_by_blog(self,id:int):
        q = select(model).where(model.blog_post_id == id)
        return (await self._sess.exec(q)).all()
